...
- `css/` — estilos divididos em 8 arquivos (ordem de carga no `<head>` segue a cascata):
  - `base.css` — `@font-face` (LT Superior), `:root`/tokens, reset, tipografia utilitária, estrutura, botões, focus-visible.
...
- `assets/` — SVGs de marca (logos, mascotes, favicon) e JPGs de produto exportados do Figma. Sempre referência relativa. Os ícones de UI NÃO ficam aqui: são SVGs inline no `index.html` (ver regra de ícones).
- `image*.png`, `screenshot-*.png` — recortes de referência do design, não são usados pela página.
...
- **Tipografia:** LT Superior (self-hosted em `assets/font/`) para tudo.
